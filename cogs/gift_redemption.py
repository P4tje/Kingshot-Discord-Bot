...
async def start_auto_redemption(cog, giftcode, auto_alliances, *, source):
    ...
    if giftcode in cog._auto_redeem_started:
        cog.logger.info(f"GiftOps: auto-redemption already started for '{giftcode}'; skipping ({source})")
        return
...
class _StateScanProgress:
    ...
    async def _message_for(self, alliance_id):
        ...
        message = await channel.send(embed=discord.Embed(
            title=f"{theme.searchIcon} Detecting Member Kingdoms", description="...",
            color=theme.emColor1))
        self.messages[alliance_id] = message
    ...
    async def finish(self, resolved, unresolved):
        ...
        embed = discord.Embed(
            title=f"{theme.verifiedIcon} Member Kingdom Detection Complete",
            description=(
                f"{theme.upperDivider}\n"
                f"{theme.membersIcon} **Members checked:** `{self.total}`\n"
                f"{theme.verifiedIcon} **Kingdoms found:** `{resolved}`\n"
    ...
    for alliance_id, members in by_alliance.items():
        try:
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"No longer in this alliance's kingdom (transferred):\n"
                    f"{theme.upperDivider}\n{listed}\n{theme.lowerDivider}"
                ),
                color=theme.emColor1,
            )
...
async def claim_giftcode_rewards_wos(cog, player_id, giftcode, *, skip_cache: bool = False,
    ...
    try:
        ...
        # Stale kingdom (member transferred): flag for the admin. Re-probing the game here
        # costs ~400 paced requests (25+ min) and would stall the whole redemption queue,
        # so it only ever runs as the separate, preemptible state_resolve job.
    ...
    try:
        ...
        if codes_to_validate:
            ...
            for giftcode in codes_to_validate:
                ...
                if is_valid is None:
                    # No verdict (usually an API rate-limit): stays validating, re-check soon.
        ...
        # already-known response (the results embed carries the status breakdown).
            ...
            if include_errors and failed_count > 0:
                ...
                if non_success_errors:
                    ...
                    error_descriptions = {
                        ...
                        "DB_UNAVAILABLE": f"{theme.warnIcon} **" + "{count}" + "** members couldn't be checked because the bot couldn't open its database - a limit on this computer, not a missing kingdom. Restart the bot and re-run; if it keeps happening, raise the host's open-file limit (macOS: ulimit -n 4096)."
                    }
