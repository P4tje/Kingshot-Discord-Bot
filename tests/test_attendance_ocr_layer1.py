...
@pytest.mark.parametrize("event_type,text,expected", [
    ("foundry_battle", "Personal Relic Points scrolling", True),
    ("foundry_battle", "Swordshrine control Duration: 00:43:41", True),
    ("foundry_battle", "Tri-Alliance Clash page", False),
    ("foundry_battle", "Congratulations on being selected as a Swordland Showdown combatant",
     True),
    ("canyon_clash", "Total Energy Used: 31.1M", True),
    ("canyon_clash", "Personal Point Ranking", True),
    ("canyon_clash", "selected as a combatant for Tri-Alliance Clash event", True),
    ("power_rankings", "Alliance Ranking Power Rankings", True),
    ("alliance_showdown", "Alliance Brawl Point Ranking", True),
    # Wrong fingerprint for event type
    ("foundry_battle", "Personal Point Ranking (tri-alliance)", False),
])
...
@pytest.mark.parametrize("event_type,text,kind,expected", [
    ("foundry_battle", "Personal Relic Points", "result",       True),
    ("foundry_battle", "Personal Relic Points", "registration", False),
    ("foundry_battle", "Congratulations on being selected as a Swordland Showdown combatant",
     "registration", True),
    ("foundry_battle", "Congratulations on being selected as a Swordland Showdown combatant",
     "result", False),
    ("canyon_clash", "Total Energy Used: 31.1M", "result", True),
    ("canyon_clash", "selected as a combatant for Tri-Alliance Clash", "registration", True),
])
...
@pytest.mark.parametrize("event_type,text,expected_kind", [
    ("foundry_battle", "Personal Relic Points scrolling", "result"),
    ("foundry_battle", "Swordshrine control Duration", "result"),
    ("foundry_battle", "Congratulations on being selected as a Swordland Showdown combatant",
     "registration"),
    ("canyon_clash", "Total Energy Used: 31.1M Personal Point Ranking", "result"),
    ("canyon_clash", "selected as a combatant for Tri-Alliance Clash", "registration"),
    ("canyon_clash", "[Legion 1] Please get ready to enter the [Tri-Alliance Clash]", "registration"),
])
...
_TEXT_FOUNDRY_REG = (
    "Congratulations on being selected as a Swordland Showdown [Legion 1] "
    "combatant. Get ready to fight!"
)
_TEXT_FOUNDRY_RESULT = (
    "Legion 1 Victory! Swordshrine control Duration 00:43:41. "
    "Rewards. Personal Relic Points ranking below."
)
_TEXT_CANYON_REG = (
    "You've been selected as a combatant for the upcoming Tri-Alliance Clash event."
)
_TEXT_CANYON_RESULT = (
    "Legion 2 ranked No. 2 in [Tri-Alliance Clash]. Total Energy Used 31.1M. "
    "Personal Point Ranking attached."
)
_TEXT_POWER_RANKING = "Alliance Ranking · Power Rankings tab"
_TEXT_SHOWDOWN = "Alliance Brawl Final Standings Point Ranking"
...
_TEXT_CANYON_REG_INVITATION = (
    "[Legion 1] Please get ready to enter the [Tri-Alliance Clash] battlefield "
...
_TEXT_FOUNDRY_REG_INVITATION = (
    "[Legion 2] Please get ready to enter the [Swordland Showdown] arena. "
...
def test_canyon_clash_fingerprint_still_matches_real_result_mails():
    ...
    assert parsers.classify_event(_TEXT_CANYON_RESULT, _ALL_EVENTS) == ("canyon_clash", "result")
    text = (
        "Congratulations, [Legion 2] of your alliance ranked No. 2 in [Tri-Alliance Clash]! "
...
def test_parse_stats_panel_partial_when_some_missing():
    text = "Total Energy Used: 31.1M Enemy Squads Defeated: 430"
    stats = parsers._parse_stats_panel(text)
...
def test_parse_mvps_simple_pairs():
    text = (
        "Total Energy Used: 19.9M marshmallow: 3.7M "
